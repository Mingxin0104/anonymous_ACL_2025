...
class ReverseLayerF(Function):
    @staticmethod
    def forward(ctx, x):
        # Retrieve lambda value from ctx, or default to 1.0
        lambd = ctx.lambd if hasattr(ctx, 'lambd') else 1.0
...
class CNN_Fusion(nn.Module):
    def __init__(self, args, W):
        ...
        vgg_19.load_state_dict(torch.load(''))
