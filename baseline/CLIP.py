...
class CLIPClassifier(nn.Module):
    def __init__(self, clip_model, num_classes):
        ...
        self.classifier = nn.Linear(512, num_classes)  # 使用FP32
    ...
    def forward(self, image, text):
        # 批量处理文本
        text = text.squeeze(1) if text.dim() == 3 else text  # 处理可能的额外维度
        text_features = self.clip_model.encode_text(text).float()  # 确保是float32
        image_features = self.clip_model.encode_image(image).float()  # 确保是float32
...
class PostDataset(Dataset):
    def __init__(self, posts, labels, image_ids, image_folder, max_length=77):
        ...
        _, self.preprocess = clip.load("ViT-B/32", device='cpu')  # 使用CPU进行预处理
    ...
    def __getitem__(self, idx):
        ...
        # 文本处理保持在CPU
        text_tensor = clip.tokenize([text], truncate=True)[0]  # 形状 (77,)

        # 图像处理保持在CPU
        image_path = f"{self.image_folder}/{image_id}"
        try:
            image = Image.open(image_path).convert("RGB")
            image = self.preprocess(image).float()  # 确保图像为float32
        except Exception as e:
            print(f"Error loading image {image_path}: {e}")
            image = torch.zeros(3, 224, 224).float()  # 确保图像为float32
...
def calculate_metrics(test_df, test_preds, test_labels):
    ...
    for field, group in field_groups:
        field_indices = group.index
        field_preds = [test_preds[i] for i in range(len(test_preds)) if i in field_indices]
        field_labels = [test_labels[i] for i in range(len(test_labels)) if i in field_indices]

        field_accuracy = accuracy_score(field_labels, field_preds)
        field_f1 = f1_score(field_labels, field_preds, average='binary')

        print(f"\nField: {field}")
        print(f"Accuracy: {field_accuracy:.4f}")
        print(f"F1: {field_f1:.4f}")

    print("\n=== Metrics by Source ===")
    source_groups = test_df.groupby('source')
    for source, group in source_groups:
        source_indices = group.index
        source_preds = [test_preds[i] for i in range(len(test_preds)) if i in source_indices]
        source_labels = [test_labels[i] for i in range(len(test_labels)) if i in source_indices]
...
def train_and_evaluate():
    ...
    model = CLIPClassifier(clip_model, num_classes).to(device).float()  # 确保模型为float32
    ...
    for epoch in range(10):
        ...
        for batch in tqdm(train_loader, desc=f'Epoch {epoch + 1}'):
            ...
            # 梯度裁剪
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)  # 调整梯度裁剪阈值
    ...
    print("\n=== Testing Best Model ===")
    # 加载分类器部分的参数，并设置 weights_only=True
