...
from models.coattention import *
from .layers import *
...
class SVFENDModel(torch.nn.Module):
    ...
    def forward(self,  **kwargs):
        ...
        fea_audio = audioframes #(batch, frames, 128)
        ...
        comments_likes=kwargs['comments_like']
        comments_feature=[]
        ...
        fea_comments =[]
        for v in range(comments_likes.shape[0]): 
            comments_weight=torch.stack([torch.true_divide((i+1),(comments_likes[v].shape[0]+comments_likes[v].sum())) for i in comments_likes[v]])
