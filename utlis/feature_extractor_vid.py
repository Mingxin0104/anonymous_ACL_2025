...
# 		f = h5py.File(os.path.join(OUTPUT_DIR, video_name + '.hdf5'), 'w')
...
#     for video_name1 in video_list:
#         video_name = video_name1 + '.mp4'
#         frame_path = os.path.join(VIDEO_DIR, video_name[:-4])  # 假设帧文件夹名和视频文件名去掉后缀一致
#         if not os.path.exists(frame_path):
#             error_fid.write(video_name + '\n')
#             print('Frame folder not found for video: %s' % video_name)
#             continue
...
#         if total_frames == 0:
#             error_fid.write(video_name + '\n')
#             print('No frames found in folder for video: %s' % video_name)
#             continue
...
#     parser.add_argument('-l', '--EXTRACTED_LAYER', dest='EXTRACTED_LAYER', type=int, choices=[5, 6, 7], default=6,
#                         help='Feature extractor layer')
#     parser.add_argument('-i', '--VIDEO_DIR', dest='VIDEO_DIR', type=str, help='Directory where frame folders are located')
#     parser.add_argument('-gpu', '--gpu', dest='GPU', action='store_true', help='Run GPU?')
#     parser.add_argument('-b', '--BATCH_SIZE', default=30, help='the batch size')
#     parser.add_argument('-id', '--gpu_id', default=0, type=int)
#     parser.add_argument('-p', '--video_list_file', type=str, help='the video name list')
